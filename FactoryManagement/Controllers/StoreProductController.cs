...
namespace FactoryManagement.Controllers
{
    ...
    public class StoreProductController : Controller
    {
        ...
        public JsonResult InventoryLocSave(Import_History model, string AllLocId, decimal[] AllQuan, decimal[] AllMinQuan, int UnitId)
        {
            if (AllLocId != "")
            {
                ...
                if (db.Import_History.Any() && db.Import_History.Where(m => m.L_C == model.L_C && m.InventoryId == model.InventoryId).Any())
                {
                    ...
                    {
                        lastCountDis += 1;
                        model.VoucherName = "IMV_" + ((lastCountDis < 1000) ? lastCountDis.ToString("000") : lastCountDis.ToString());
                        model.ParentId = imp_his.ImportId;
                        try
                        {
        ...
        public PartialViewResult _InventoryMinQuanFinished(string v, string searchId)
        {
            var list = db.View_ProList_WithLC.ToList();
            var storeList = list.Where(m => m.StoreId != null && m.StoreId > 0).GroupBy(m => new
            {
                m.InventoryId,
                m.StoreId
            }).Select(i => new DispatchedItemListModelView
            {
            ...
            var warelist = list.Where(c => c.WarehouseId > 0 && c.WarehouseId != null).GroupBy(c => new { c.WarehouseId, c.InventoryId })
                .Select(i => new DispatchedItemListModelView
                {
                    InventoryId = i.FirstOrDefault().InventoryId,
                    LocationId = i.FirstOrDefault().LocationId,
            ...
            var allIds = storeList.Where(x => x.Quantity <= x.MinimumQuantity).Select(x => x.InventoryId).Distinct().ToList();
            List<DispatchedItemListModelView> dispatchItemList = new List<DispatchedItemListModelView>();
            foreach (var id in allIds)
            {
                var Inventory = list.Where(x => x.InventoryId == id).Select(x => new DispatchedItemListModelView
                {
                    InventoryId = x.InventoryId,
                    Quantity = list.Where(c => c.InventoryId == x.InventoryId).Sum(c => c.Quantity),
                    MinimumQuantity = x.MinimumQuantity,
                    Productname = x.ProductName
                }).FirstOrDefault();
                dispatchItemList.Add(Inventory);
            }
            dispatchItemList = dispatchItemList.OrderBy(x => x.Productname).ToList();
            Session["InventSearchList"] = dispatchItemList;
            if (!string.IsNullOrEmpty(searchId))
            {
                List<int> MinList = new List<int>();
                foreach (var id in searchId.Split(',').ToList())
                {
                    MinList.Add(Convert.ToInt32(id));
                }
                dispatchItemList = dispatchItemList.Where(x => MinList.Contains(x.InventoryId)).ToList();
            }

            Session["InventList"] = dispatchItemList;
            Session["InventListCount"] = dispatchItemList.Count();
            ViewBag.Total = dispatchItemList.Count();
            dispatchItemList = dispatchItemList.Take(ShowItemPerPage).ToList();

            return PartialView(dispatchItemList);
        }
        public PartialViewResult _ViewInventoryDetails(int? id)
        {
            var list = db.View_ProList_WithLC.Where(m => m.InventoryId == id).ToList();
            var storeList = list.Where(m => m.StoreId != null && m.StoreId > 0).GroupBy(m => new
            {
                m.InventoryId,
                m.StoreId
            }).Select(i => new DispatchedItemListModelView
            {
                ...
                Unitname = i.FirstOrDefault().UnitName,
                WareId = i.FirstOrDefault().WarehouseId,
                Storename = i.FirstOrDefault().StoreName,
                Productname = i.FirstOrDefault().ProductName,
                Quantity = i.Sum(x => x.Quantity),
                MinimumQuantity = i.FirstOrDefault().MinimumQuantity,
            }).ToList();
            ...
            var warelist = list.Where(c => c.WarehouseId > 0 && c.WarehouseId != null).GroupBy(c => new { c.WarehouseId, c.InventoryId })
                .Select(i => new DispatchedItemListModelView
                {
                    ...
                    Unitname = i.FirstOrDefault().UnitName,
                    WareId = i.FirstOrDefault().WarehouseId,
                    Warename = i.FirstOrDefault().WareName,
                    Productname = i.FirstOrDefault().ProductName,
                    Quantity = i.Sum(x => x.Quantity),
            ...
            storeList = storeList.OrderBy(x => x.InventoryId).ToList();
            return PartialView(storeList);
        }
        public JsonResult ScrollFinishedMinInventory(int? page)
        {
            page = page ?? 0;
            var total = Convert.ToInt32(Session["InventListCount"]);
            int skip = ShowItemPerPage * Convert.ToInt32(page);
            var canPage = skip < total;
            if (canPage)
            {
                var listBackFromTotal = (List<DispatchedItemListModelView>)Session["InventList"];
                var InventMinFinished = listBackFromTotal.Skip(skip).Take(ShowItemPerPage).ToList();
                return Json(InventMinFinished, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        public JsonResult SearchFinishedMinInventory()
        {
            var dispatchList = (List<DispatchedItemListModelView>)Session["InventSearchList"];
            foreach (var names in dispatchList)
            {
                if (!dispatchList.Where(m => m.Productname == names.Productname).Any())
                {
                    DispatchedItemListModelView dispatchItemNameList = new DispatchedItemListModelView();
                    dispatchItemNameList.Productname = names.Productname;
                    dispatchItemNameList.InventoryId = names.InventoryId;
                    dispatchList.Add(dispatchItemNameList);
                }
            }
            return Json(dispatchList, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult LcNameSearching(long prefix)
        {
            if (prefix <=0)
            {
                return Json("", JsonRequestBehavior.AllowGet);
            ...
            {
                var allproduct = db.View_ProList_WithLC.Where(x => x.InventoryId == prefix).GroupBy(x=>x.LC).Select(c => new
                {
                    Import=c.FirstOrDefault().ImportId,
                    Lc=c.FirstOrDefault().LC
        ...
        public PartialViewResult InvntLocMinQuanDetailsPopup(int id)
        {
            if (id > 0)
            {
                var model = db.View_All_InventoryList.Where(m => m.InventoryId == id).FirstOrDefault();
        ...
        //    return PartialView();
        //}
        public JsonResult InventoryLocMinQuanChange(int id, int MinimumQuantity, string type, int UserId, int InventoryId)
        {
            string msg = "";
            string opname = "Inventory Minimum Quantity Change";
            int ColumnId = 0;
            int OperationStatus = 1;
            if (id > 0 && MinimumQuantity >= 0 && (!string.IsNullOrEmpty(type)) && InventoryId > 0)
            {
                string itemType = type;
                List<Inventory_Item_Location> inventLoc = itemType == "S" ? db.Inventory_Item_Location.Where(x => x.InventoryId == InventoryId && x.StoreId == id).ToList() : db.Inventory_Item_Location.Where(x => x.InventoryId == InventoryId && x.WarehouseId == id).ToList();
                if (inventLoc.Count() > 0)
                {
                    try
                    {
                        foreach (var item in inventLoc)
                        {
                        ...
                        msg = "Inventory minimum quantity has been successfully changed on " + now.ToString("MMM dd , yyy hh:mm tt") + " .";
                        ColumnId = InventoryId;
                    }
                    catch (Exception)
                    {
                        msg = "Inventory minimum quantity change was unsuccessfully on " + now.ToString("MMM dd , yyy hh:mm tt") + " .";
                        ColumnId = 0;
                        OperationStatus = -1;
                    }
                    SaveAuditLog(opname, msg, "Inventory", "StoreProductDetails", "Inventory_Item_Location", ColumnId, UserId, now, OperationStatus);
                }
                else if (inventLoc.Count() == 0)
                {
                    Inventory_Item_Location item_location = new Inventory_Item_Location();
                    item_location.InventoryId = InventoryId;
                    if (type == "S")
                    {
                        item_location.StoreId = id;
                    }
                    else
                    {
                        item_location.WarehouseId = id;
                    }
                    item_location.Quantity = 0;
                    item_location.MinimumQuantity = MinimumQuantity;
                    item_location.UpdatedBy = UserId;
                    item_location.UpdatedDate = now;
                    db.Inventory_Item_Location.Add(item_location);
                    try
                    {
                        db.SaveChanges();
                        msg = "Inventory item created for minimum quantity has been successfully inserted on " + now.ToString("MMM dd , yyy hh:mm tt") + " .";
                        ColumnId = InventoryId;
                    }
                    catch (Exception)
                    {
                        msg = "Inventory item created for minimum quantity change was unsuccessfully on " + now.ToString("MMM dd , yyy hh:mm tt") + " .";
                        ColumnId = 0;
                        OperationStatus = -1; ;
                    }
                    SaveAuditLog(opname, msg, "Inventory", "StoreProductDetails", "Inventory_Item_Location", ColumnId, UserId, now, OperationStatus);
        ...
        public PartialViewResult StoreProductDetailsPopUp(int id)
        {
            if (id > 0)
            {
                var model = db.View_ProList_WithLC.Where(m => m.InventoryId == id && (m.LC == "N/A" || string.IsNullOrEmpty(m.LC))).FirstOrDefault();
                return PartialView(model);
            }
            return PartialView();
        }
        ...
        public PartialViewResult _ProLCHis(int id)
        {
        ...
        public ActionResult _StoreList(List<string> locations, int? productId, int? storeid, int? wareid, int viewId)
        {
            List<int> locationId = new List<int>();
            List<string> Lcs = new List<string>();
            if (locations != null && locations.Count() > 0) {
                foreach (var item in locations) {
                    locationId.Add(Convert.ToInt32(item.Split('-')[0]));
                    if(item.Split('-')[1]!="" && item.Split('-')[1] != "N/A"){
                        Lcs.Add(item.Split('-')[1]);
                    }
                }
            }
            var list = db.View_ProList_WithLC.ToList();

            var allProduct = (locationId.Count() <= 0 && Lcs.Count() <= 0) ? (from bs in list
                              ...
                              : (locationId.Count() > 0 && Lcs.Count() > 0) ? list.Where(x => locationId.Contains(x.LocationId) && Lcs.Contains(x.LC))
                              .GroupBy(x => new { x.InventoryId, x.LC })
                              .Select(i => new InventoryListModelView
                              {
                                  InventoryId = i.FirstOrDefault().InventoryId,
                                  ImportId = i.FirstOrDefault().ImportId,
                                  LC = i.FirstOrDefault().LC,
                              ...
                              : list.Where(x => locationId.Contains(x.LocationId))
                              .GroupBy(x => new { x.InventoryId, x.LC })
                              .Select(i => new InventoryListModelView
                              {
                                  InventoryId = i.FirstOrDefault().InventoryId,
            ...
            //{
            //    //List<string> productLc = list.Where(x => locationId.Contains(x.LocationId) && x.LC != "N/A").Select(x => x.LC).ToList();
            //    //if (productLc.Count() > 0)
            //    //{
            //    //    allProduct = allProduct.Where(x => locationId.Contains(x.LocationId) || productLc.Contains(x.LC)).ToList();
            //    //    /*allProduct.Where(x=> locationIds.Contains(x.LocationId)).ToList();*/
            //    //    ViewBag.Count = allProduct.Count();
            //    //}
            //    //else
            //    //{
            //        allProduct = allProduct.Where(x => locationId.Contains(x.InventoryId)).ToList();
            //        /*allProduct.Where(x=> locationIds.Contains(x.LocationId)).ToList();*/
            //        ViewBag.Count = allProduct.Count();
            //    //}
        ...
        public JsonResult GetAllProductNames(int? tab)
        {
            var list = (from i in db.View_ProList_WithLC
                        ...
                        select new InventoryListModelView
                        {
                            ...
                            ProNameWithLc = m.FirstOrDefault().ProductName + (tab == 3 ? (" ( L/C :" + m.FirstOrDefault().LC + " )") : ""),
                            ProductName = m.FirstOrDefault().ProductName,
                            ProductTypeId = m.FirstOrDefault().ProductTypeId
                        }).OrderBy(o => o.ProductName).ToList();
            list = tab == 1 ? list.ToList() : (tab == 2 ? list.Where(i => i.LC == "N/A").ToList() : list.Where(i => i.LC != "N/A").ToList());
            return Json(list, JsonRequestBehavior.AllowGet);
        }
        ...
        public PartialViewResult _InvenDisVoucherList(bool? isStore, int? AssignType, int? days, DateTime? from, DateTime? to)
        {
            ...
            {
                list = list.Where(m => m.DispatchedDate.Date >= from.Value.Date && m.DispatchedDate.Date <= to.Value.Date).OrderByDescending(m => m.DispatchedDate).ToList();
            }
            Session["AllInvenDisVouList"] = list;
            Session["TotalInvenDisVouCount"] = list.Count();
            ViewBag.tab = AssignType;
        ...
        public PartialViewResult _InventoryItemDispatchHisDayWise(int? days, DateTime? from, DateTime? to)
        {
            ...
            if (days > 0)
            {
                ...
                int day = Convert.ToInt32(days);
                countDate = countDate.AddDays(-(day));
                AllList = AllList.Where(m => m.DispatchedDate.Value.Date > countDate).OrderByDescending(m => m.DispatchedDate).ToList();
            }
            if (from != null && to != null)
            {
                AllList = AllList.Where(m => m.DispatchedDate.Value.Date >= from.Value.Date && m.DispatchedDate.Value.Date <= to.Value.Date).OrderByDescending(m => m.DispatchedDate).ToList();
            }
        ...
        public JsonResult GetInvenItemDisHis(int? page)
        {
            ...
            int skip = 5 * Convert.ToInt16(page);
            var canPage = skip < total;
            if (canPage)
            {
        ...
        public ActionResult LocationWiseRoutedHistory() {
            var locationList = db.View_All_StoreWarehouseLists.ToList();
            return View(locationList);
        }
        public PartialViewResult _LocationWiseRoutedHistory(string location) {
            if (string.IsNullOrEmpty(location))
            {
                ...
                var locationId = Convert.ToInt32(location.Split('-')[0]);
                ViewBag.locationId = locationId;
                if (location.Split('-').Last() == "s")
                {
                    var list = db.View_Routedhistory.Where(x => x.StoreId == locationId).ToList();
                    return PartialView(list);
                }
                else if (location.Split('-').Last() == "w") {
                    var list = db.View_Routedhistory.Where(x => x.WareId == locationId).ToList();
                    return PartialView(list);
                }
            }
            return PartialView();
        }
        public ActionResult ProductWiseRoutedHistory() {
            return View();
        }
        
        public PartialViewResult ShowInvenRouteQuanDetails(int invenid, int? DisId, int? days,int? locationId)
        {
            ViewBag.locationId = locationId;
            if (DisId == null)
            {
        ...
        public PartialViewResult _ImportHistoryList(int? days, DateTime? from, DateTime? to, string[] search)
        {
            ...
            if (search != null)
            {
                ...
                foreach (var item in search)
                {
                    if (item.Split('-')[1] == "p")
                    {
                        long id = Convert.ToInt64(item.Split('-')[0]);
                        list.AddRange(db.View_Import_History_Details.Where(m => m.InventoryId == id).OrderByDescending(m => m.InsertedDate).ToList());
                    }
                    else if (item.Split('-')[1] == "t")
                    {
                        long id = Convert.ToInt64(item.Split('-')[0]);
                        list.Add(db.View_Import_History_Details.Where(m => m.ImportId == id).FirstOrDefault());
                    }
                    else if (item.Split('-').Last() == "l")
                    {
                        string lc = item.Substring(0, item.Length - 2);
                        list.AddRange(db.View_Import_History_Details.Where(m => m.L_C == lc).OrderByDescending(m => m.InsertedDate).ToList());
                    }
                }
            }
            if (days > 0)
            {
                ...
                int day = Convert.ToInt32(days);
                countDate = countDate.AddDays(-(day));
                list = list.Where(m => m.InsertedDate.Date > countDate).OrderByDescending(m => m.InsertedDate).ToList();
            }
            if (from != null && to != null)
            {
                list = list.Where(m => m.InsertedDate.Date >= from.Value.Date && m.InsertedDate.Date <= to.Value.Date).OrderByDescending(m => m.InsertedDate).ToList();
            }
            Session["AllImportHistoryList"] = list;
            Session["TotalImportHistoryCount"] = list.Count();
            ViewBag.Count = list.Count();
            list = list.Take(ShowItemPerPage).ToList();
            return PartialView(list);
        }
        public PartialViewResult _ImportHistoryLCList(int? days, DateTime? from, DateTime? to, string[] search)
        {
        ...
        public JsonResult GetImportHistory(int? page, int tab)
        {
            ...
            if (tab == 1)
            {
                ...
                if (canPage)
                {
                    ...
                    var list = listBackFromSession.Skip(skip).Take(ShowItemPerPage).ToList();
                    return Json(list, JsonRequestBehavior.AllowGet);
                }
                else
                {
        ...
        public JsonResult GetAllImportHistory()
        {
            List<SearchModelView> searchList = new List<SearchModelView>();
            var list = (from i in db.View_Import_History_Details select i).ToList();
            foreach (var data in list)
            {
                if (!searchList.Where(m => m.SearchTag == data.ProductName).Any())
                {
                    SearchModelView searchView = new SearchModelView();
                    searchView.Id = data.InventoryId + "-p";
                    searchView.SearchTag = data.ProductName;
                    searchList.Add(searchView);
                }

            }
            foreach (var data in list)
            {
                if (!searchList.Where(m => m.SearchTag == data.VoucherName).Any())
                {
                    SearchModelView searchView = new SearchModelView();
                    searchView.Id = data.ImportId + "-t";
                    searchView.SearchTag = data.VoucherName;
                    searchList.Add(searchView);
                }
            ...
            foreach (var data in list)
            {
                if (!searchList.Where(m => m.SearchTag == data.L_C).Any())
                {
                    SearchModelView searchView = new SearchModelView();
                    searchView.Id = data.L_C + "-l";
                    searchView.SearchTag = data.L_C;
                    searchList.Add(searchView);
                }
        ...
        public PartialViewResult _InventoryItemDispatchHisMachineWise(int? days, DateTime? from, DateTime? to)
        {
            var allList = db.View_Dispacth_Item_Details.ToList();
            if (days > 0)
            {
            ...
            var list = (from bs in allList
                        ...
                        select new View_Dispacth_Item_DetailsModelView
                        {
                            ...
                            Total_Return = g.Sum(x => x.ReturnQuantity.Value),
                            Total_UsedQuan = g.Sum(x => x.UsedQuan.Value)
                        }).OrderByDescending(m => m.DispatchedDate).ToList();
        ...
        public ActionResult _DeptWiseDispatchList(int? dId, int? days, string v, DateTime? from, DateTime? to)
        {
            var allList = db.View_Dispacth_Item_Details.ToList();
            if (dId > 0)
            {
                allList = allList.Where(m => m.DeptId == dId).ToList();
            }
            if (days > 0)
            {
                DateTime countDate = now.Date;
            ...
            var list = allList.Where(m => m.DeptId > 0)
                        .GroupBy(m => new { m.DeptId, m.InventoryId })
                        .Select(g => new View_Dispacth_Item_DetailsModelView
                        {
                            Id = g.FirstOrDefault().Id,
                            DispatchedId = g.FirstOrDefault().DispatchedId,
                            Name = g.FirstOrDefault().Name,
            ...
            ViewBag.tab = dId;
            ViewBag.Count = list.Count();
            ViewBag.list = list.Take(DisMAchineWiseHisPerPage).ToList();//DisMAchineWiseHisPerPage
            return PartialView();
        }
        public JsonResult GetDeptDisHis(int? page)
        {
            ...
            int skip = 1 * Convert.ToInt16(page);
            var canPage = skip < total;
            if (canPage)
            {
                var listBackFromSession = (List<View_Dispacth_Item_DetailsModelView>)Session["AllDeptDisHisList"];
                var partslist = listBackFromSession.Skip(skip).Take(1).ToList();
                return Json(partslist, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        //replaced with nazrul vai code
        public PartialViewResult InventoryProductListByTypePartial(List<int?> locationId, int? productTypeId, int? page, int? tab)
        {
            ViewBag.productTypeId = productTypeId;
            ViewBag.page = page;
            ViewBag.tab = tab;
            if (locationId != null)
            {
                var data = db.View_ProList_WithLC.Where(x => locationId.Contains(x.LocationId)).OrderBy(x => x.ProductName).ToList();
            ...
            if (tab != null && tab > 1)
            {
                var data = tab == 2 ? db.View_ProList_WithLC.Where(x => x.LC == "N/A").OrderBy(x => x.ProductName).ToList() : db.View_ProList_WithLC.Where(x => x.LC != "N/A").OrderBy(x => x.ProductName).ToList();
                data = productTypeId > 0 ? data.Where(x => x.ProductTypeId == productTypeId).OrderBy(x => x.ProductName).ToList() : data;
                Session["AllProList"] = data;
                Session["ProListCount"] = data.Count();
                ViewBag.totalCount = data.Count();
                return PartialView(data.Take(ShowItemPerPage));
            }
            else if (productTypeId == null || productTypeId <= 0)
            {
                var data = db.View_ProList_WithLC.OrderBy(x => x.ProductName).ToList();
                Session["AllProList"] = data;
                Session["ProListCount"] = data.Count();
                ViewBag.totalCount = data.Count();
                return PartialView(data.Take(ShowItemPerPage));
            }
            var data2 = db.View_ProList_WithLC.Where(x => x.ProductTypeId == productTypeId).OrderBy(x => x.ProductName).ToList();
        ...
        public JsonResult getProListonScroll(int? page)
        {
            ...
            int skip = ShowItemPerPage * Convert.ToInt16(page);
            var canPage = skip < total;
            var listBackFromSession = (List<View_ProList_WithLC>)Session["AllProList"];
        ...
        public PartialViewResult _InvenProListForWaste(string idList)
        {
            List<int> id = new List<int>();
            var lidtofId =(string.IsNullOrEmpty(idList))?null: idList.Split(',');
            if (!string.IsNullOrEmpty(idList) && lidtofId.Length > 0  ) {
                for (int i = 0; i < lidtofId.Length; i++) {
                    id.Add(Convert.ToInt32(lidtofId[i]));
                }
            }
            ...
            if (id.Count() > 0)
            {
                list = list.Where(m =>id.Contains(m.InventoryId)).ToList();
            }
            return PartialView(list);
        }
        ...
        public PartialViewResult _InvenMcsWasteList(int type, int order, string idList, DateTime? fromDate, DateTime? toDate)
        {
            List<long> InventoryId = new List<long>();
            if (!string.IsNullOrEmpty(idList)) {
                var ids = idList.Split(',');
                for (int i = 0; i < ids.Length; i++) {
                    InventoryId.Add(Convert.ToInt64(ids[i]));
                }
            }
            ViewBag.type = type;
            ViewBag.order = order;
            var list = (from c in db.Inventory_Pro_Waste_List
                        ...
                        select new Inventory_Pro_Waste_ListModelView
                        {
                            Id = g.FirstOrDefault().Id,
                            InventoryId = g.FirstOrDefault().InventoryId,
                            ProductName = db.InventoryLists.Where(u => u.InventoryId == g.FirstOrDefault().InventoryId).Select(u => u.ProductName).FirstOrDefault(),
                            LocationId = g.FirstOrDefault().LocationId,
